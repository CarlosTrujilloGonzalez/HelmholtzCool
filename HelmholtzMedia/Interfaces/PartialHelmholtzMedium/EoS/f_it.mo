...
algorithm
  f_ideal_tau :=
    ...
    + sum(eg[i,1]*eg[i,4]*eg[i,3]*exp(eg[i,4]*tau)/(eg[i,2]+eg[i,3]*exp(eg[i,4]*tau)) for i in 1:nEinsteinGeneralized)
    - sum(c[i,1]*c[i,2]*tanh(c[i,2]*tau) for i in 1:nCosh)
    + sum(s[i,1]*s[i,2]/tanh(s[i,2]*tau) for i in 1:nSinh)
    +k/tau
    +sum(cp[i,1]*Tc^cp[i,2]*tau^(-cp[i,2]-1)/(cp[i,2]+1) for i in 1:nCpPolyT);
end f_it;
